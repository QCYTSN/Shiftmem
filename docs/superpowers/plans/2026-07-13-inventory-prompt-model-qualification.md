...
- [ ] Write tests asserting reset has empty history/public costs, completed records enter history chronologically, the window truncates to 14, pipeline due dates are visible, and forbidden Oracle/shift keys are absent.
- [ ] Run `.venv\Scripts\python.exe -m pytest tests/unit/test_inventory_env.py -q` and observe failures for missing keys.
- [ ] Implement `_pipeline_orders()` and build the richer observation without changing transition order or record contents. Update Agent/fallback observation annotations to `dict[str, Any]`.
- [ ] Re-run the inventory and structured-agent tests and require all to pass.
...
- [ ] Write failing tests that inspect the fake HTTP payload and require the prompt to name the cost objective, inventory/pipeline accounting, recent history, fallible memory, supplied-ID rule, hidden-state prohibition, and exact JSON schema.
- [ ] Run the provider test file and verify the prompt assertions fail.
- [ ] Implement the focused prompt module and use it from `CompatibleAPIProvider`; user serialization stays deterministic and includes correction feedback when present.
- [ ] Re-run provider and structured-agent tests and require all to pass.
...
- [ ] Write failing tests for deterministic case IDs, identical non-target fields within pairs, dormant-memory metadata, monotonic gate calculation, schema/fallback accounting, and aggregate qualification status.
- [ ] Run the new test file and observe import/behavior failures.
- [ ] Implement Pydantic result schemas, eight deterministic cases, and pure aggregate logic. A model qualifies only when both repetitions parse without fallback or invalid citations and all paired comparisons pass.
- [ ] Re-run the new tests and require all to pass.
...
- [ ] Write failing tests using a fake provider factory to verify exact model/profile selection, two repetitions, unavailable/error recording, no credential fields, and aggregate output.
- [ ] Run the new runner tests and observe import/behavior failures.
- [ ] Implement CLI arguments `--config`, `--raw-output`, and `--summary-output`; configure Flash, open Qwen 35B-A3B, DeepSeek V3.2, and GLM-5.1 without embedding keys.
- [ ] Re-run runner tests, then the full offline suite and compilation.
- [ ] Run the bounded live qualification once, inspect hard gates, update the implementation log/model recommendation, and verify raw output and `.env` are ignored and no populated key is present in repository text.
