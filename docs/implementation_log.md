...
- Repaired the Development/Validation v2 live-Pilot runner before spending its separately approved CNY 6 / 350-call budget. The runner now requires a clean hash-verified harness, run-specific output paths, an fsynced per-attempt journal with replay, complete model pricing and token ceilings, a cost-derived effective call cap, provenance metadata, atomic aggregate updates, and separate scheduler/review/daily-strategy/order/outcome logs. A 10-cell offline CLI smoke completed 303 deterministic reviews with full logs; the full network-free suite passed 314 tests. No provider call was made during this repair.
- Completed the frozen live Pilot `v2-live-pilot-20260715-a148a81`: 8/8 Validation cells, 240 reviews, 272 attempts, 915,521 input and 114,303 output tokens, four fallbacks, and a successful-response token estimate of CNY 3.7710 under the CNY 6 cap. All 36 attempt errors were provider failures and none were JSON/schema errors. DeepSeek had no failure; MiniMax had 36 failures in 152 attempts. The outer terminal timed out after 60 minutes, but the still-running process completed from the fsynced journal without a second process or repeated request. The Pilot exposed a conformance limitation: ShiftMem used current defaults rather than the previously Validation-selected detector/dormancy/retrieval profile, so cost/reliability evidence is retained but protocol finalization and formal performance inference remain blocked.
- Removed implicit ShiftMem runtime defaults from future v2 Pilot/formal paths. `make_memory` now accepts an explicit validated ShiftMem/retrieval profile; Pilot configs and the formal-v2 proposal carry Page-Hinkley 10/0.1/48, validation window 3, dormancy 3, and recency-heavy weights, while the formal controller defaults/bounds and scheduler fields are machine-validated against the implementation. The completed live-Pilot config is marked `completed_no_rerun` with budget approval disabled. A 40-cell network-free readiness rerun under the explicit profile measured 30.0 baseline and 34.1 ShiftMem reviews per episode; the revised provisional envelope is 20,000 attempts, 90M input tokens, 7M output tokens, and CNY 360, all unapproved. Validation-observed maximum review changes (forecast window 7, safety multiplier 1.0, buffer 1) are now deterministic delta caps; they cover every recorded live-Pilot revision, so no live rerun is required.
- Added the current protocol-v2 formal-readiness audit. Qualification, Pilot cost/reliability evidence, explicit profiles, delta caps, and offline readiness now pass, but Test remains blocked because the formal CLI still executes only the retired v1-shaped dry-run path, a freeze-bound 480-cell v2 executor does not yet exist, the CNY 360 envelope is unapproved, protocol remains draft, and no replacement freeze exists.

## 2026-07-16 - Protocol 2.0 Amendment 1 and reduced formal budget

- The user explicitly approved a CNY 100 hard cap before any Test outcome access. Amendment 1 retains both qualified models, all eight held-out scenarios, and the primary ShiftMem-versus-VectorMemory estimand, reduces paired seeds from ten to five, and removes the paid secondary tier. The final paid matrix is 160 cells; H2 becomes exploratory.
...
## 2026-07-22 - Formal completion and post-Test closure

- Completed all 160 paid primary cells: 80 Test-ID and 80 Test-OOD, with 70 paired non-stable units for the primary endpoint and no unresolved journal reservation.
- Added `formal_v2_post_test.yaml` and `finalize_formal_results.py` to bind five immutable cell prefixes, four journals, two summaries, the analysis code and contracts, and four source freezes; the resulting evidence ID is `v2-formal-results-bf7b57070da2`.
- Added deterministic machine-readable statistical and reliability aggregates. The primary calculation keeps the declared ShiftMem-minus-VectorMemory direction and the predeclared paired-test selection logic; no table, figure, paper claim, or new provider call is produced by the closure command.
- Preserved every raw result byte. The derived evidence manifest discloses that all 160 cell records and the Test-ID summary contain a false `test_outcomes_accessed` metadata default, records the corrected post-Test interpretation, and states that numeric outcomes are unaffected. Future held-out cell serialization now sets the field explicitly.
