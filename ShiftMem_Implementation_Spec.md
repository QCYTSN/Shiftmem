...
> **预计周期**：14–16 周  
> **核心定位**：冻结基础语言模型，以外部变化感知条件记忆辅助低频策略复盘，由确定性控制器执行每日补货。
> **文档状态**：v2.0 架构修订稿；替代未进入正式 Test 的 v1/v1.1 Agent 决策架构
...
> 当库存经营环境发生突发、渐进、周期或组合变化时，低频 LLM 策略 Agent 能否识别旧经验何时失效，并通过带适用条件、可信度和生命周期状态的外部记忆，更快修订库存控制策略？
...
1. 一个可复现的非平稳库存模拟环境；
2. ShiftMem 外部记忆模块；
3. 六类记忆基线、共享确定性库存控制器、传统策略和 Oracle 上限；
4. 完整实验、消融、泛化和成本分析；
5. 一篇结构完整的英文研究论文；
...
- 让 LLM 进行低频、事件驱动的策略复盘，不直接计算每日订货量；
- 由固定公式和有界参数组成的确定性控制器执行每日补货；
...
```mermaid
flowchart TD
    A[每日库存环境] --> B[公开状态与经营日志]
    B --> C[在线变化检测]
    C --> D{五日复盘或检测到变化}
    D -->|否| E[沿用当前策略参数]
    D -->|是| F[ShiftMem 检索适用经验]
    F --> G[LLM 策略复盘 Agent]
    G --> H[参数校验与安全裁剪]
    H --> E
    E --> I[确定性库存控制器]
...
| Review Scheduler | 日序、变化信号、冷却状态 | 是否唤醒及触发原因 | 不调用环境隐藏状态 |
| Strategy Agent | observation、当前策略、Top-k 经验 | 有界策略参数提案 | 不输出每日订货量 |
| Inventory Controller | 公开历史、库存位置、有效策略参数 | 结构化每日补货动作 | 不调用 LLM、不访问隐藏真值 |
...
需要明确“成功”的延迟窗口。v2 验证的是一次策略修订在后续完整观察窗口中的效果，而不是把某一天的订货量归因给 LLM。验证应根据后续若干天的成本、服务水平、缺货和库存积压计算。验证逻辑必须由确定性评估器完成，LLM 只负责提出或解释策略修订，不自由判断经验是否成功。
...
- 每完成 5 个环境日进行一次周期复盘；
- 相关需求或供应变化信号可触发额外复盘；
- 同一天的周期触发与变化触发合并为一次调用；
- 重复检测信号受 Validation 选择并冻结的冷却窗口约束；
- 复盘间隔和冷却规则不能由 LLM 修改。
...
- 参数默认值、上下界和最大单次改变量必须在 Validation 上选择并冻结；
- LLM 不得输出 `order_quantity`、修改控制器公式或改变复盘间隔；
- 无法解析时重试一次；
- 第二次仍失败则保留上一组有效参数并记录 fallback；
- 决策理由不参与主要经营评分；
- `used_memory_ids` 用于计算记忆错误复用率和忠实度。

### 7.3 确定性每日控制器

控制器使用公开需求历史、公开 quoted lead time、库存位置和当前有效策略参数，计算每日 order-up-to 目标与非负整数订货量。所有 LLM 记忆方法共享完全相同的控制器实现、初始参数、参数边界和 fallback。控制器不得调用 LLM，也不得读取真实需求参数、隐藏 regime ID、未来需求或 Oracle 信息。
...
六种方法都只影响低频策略复盘所见的历史上下文，不直接影响每日控制公式：

1. `NoMemoryAgent`：策略复盘只看当前公开状态和固定历史窗口；
2. `FullHistoryAgent`：在上下文预算允许范围内提供最近完整历史；
3. `SummaryMemoryAgent`：维护滚动自然语言摘要；
4. `VectorMemoryAgent`：按语义相似度检索 Top-k 策略经验；
5. `TimeDecayMemoryAgent`：相关度结合时间衰减；
6. `ShiftMemAgent`：完整的条件检索、验证和生命周期管理。
...
- 移动平均预测 + 再订货点；
- 指数平滑预测 + 安全库存；
- 与 LLM 使用相同控制器但参数固定的策略；
- 检测触发、按预设规则调整参数的确定性适应策略；
- 已知分布参数的 Oracle 策略。

### 8.3 公平性规则

- Agent 基线使用相同基础模型；
- 每次策略复盘的最大输入 Token 和检索条数尽量一致；
- 相同情景使用相同随机种子与需求轨迹；
- 所有 LLM 方法使用相同策略参数空间、控制器、初始参数、复盘计划、触发冷却和状态信息；
- 同一天的周期复盘和变化告警只能产生一次调用；
- 记录真实 Token、延迟和失败重试；
- 开发集用于调参，测试集一次性冻结运行。
...
- 8 个已声明但尚未执行的 Test-ID/Test-OOD 场景，覆盖稳定、突发、渐进、供应、周期、虚警和组合变化；
- 10 个配对环境随机种子；
- 2 个已资格测试的核心模型；
- `VectorMemoryAgent` 与 `ShiftMemAgent` 两种主比较方法；
- 共 320 个主模型—方法单元。

**次要记忆比较**：

- `NoMemory`、`FullHistory`、`Summary` 和 `TimeDecay`；
- 只使用 DeepSeek、5 个配对种子和相同 8 个场景；
- 不将次要比较扩展成两个模型上的完整笛卡尔积。

**针对性消融**：

- H3 只使用稳定场景；
- H4 在周期复现场景比较有无 dormancy/reactivation；
- H5 在变化场景比较统计触发与 LLM-only 变化判断；
- 第三个模型、多商品和额外参数网格仅在预算允许时作为附录。

每个 episode 仍覆盖完整变化前和变化后窗口，按日计算经营结果。主实验固定为 10 个配对种子；调用上限和费用上限只有在 v2 Development/Validation Pilot 完成后才能最终冻结。原 v1.1 的 52 种子全矩阵和费用推算正式废弃，不能作为 v2 执行依据。
...
- 不同种子方差；
- 单位性能提升的推理成本。
- 周期/事件复盘次数、合并触发次数和冷却抑制次数；
- 策略参数改变量、参数 churn 和无效提案率；
- 每次成功策略修订的成本。
...
- 固定五日复盘与仅事件触发的敏感性比较，仅在 Development/Validation 预先声明后运行。
...
- 并排比较两个 Agent；
- 展示每日控制器动作，以及每次低频策略修订的参数、理由和使用的记忆；
- 展示记忆状态、可信度和状态变化原因。
...
### Phase 2：统一策略 Agent 接口与简单记忆（第5–6周）

- 实现 provider 抽象和 JSON 输出校验；
- 完成 NoMemory、FullHistory、Summary、Vector、TimeDecay；
- 实现五日/事件触发调度、策略参数 schema 和确定性每日控制器；
- 记录 Token、延迟、失败、策略修订和每日控制器动作；
- 用小模型完成短 episode 端到端运行。

**验收**：同一情景可一条命令切换全部基线；LLM 无法直接订货；解析失败时保留上一组有效策略。
...
- 小规模比较检测器和检索权重；
- 使用 Development/Validation 选择策略参数边界、触发冷却和延迟验证窗口；
- 重新验证两个核心模型能稳定输出 v2 策略 schema；
- 估计方差、推理时间和正式实验成本；
- 检查指标是否可计算；
- 冻结测试配置和随机种子清单。
...
| 风险 | 预警信号 | 降级方案 |
|---|---|---|
| LLM 推理成本过高 | Pilot 单配置耗时过长 | 五日/事件驱动复盘、分层矩阵和强制调用预算 |
...
| LLM 不遵守策略格式 | 解析失败率高 | 约束解码、一次重试并保留上一有效策略 |
| 实验组合爆炸 | 预计运行量持续增长 | 坚持主实验矩阵，扩展实验进入附录或取消 |
| 检测器重复告警 | 短期连续唤醒 LLM | 合并同日触发并冻结复盘冷却窗口 |
| 策略参数频繁震荡 | 稳定场景参数 churn 高 | 限制参数范围与单次改变量，报告稳定性指标 |
| GPU 租赁时间失控 | 下载、部署和空闲占用增长 | API 主实验保持 CPU 编排；本地模型作为单独预算的可选附录 |
